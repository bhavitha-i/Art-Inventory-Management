...
exports.create = (req, res) => {
  ...
  dbmodels.Art_Supplies.create(rowData)
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.findByPk = (req, res) => {
    dbmodels.Art_Supplies.findByPk(req.params.id, {
    })
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.update = (req, res) => {
  ...
  dbmodels.Art_Supplies.update(req.body,
    { where: { id_Art_Supplies: id } }
  ).
  then(() => {
    res.status(200).json({
        status: true,
