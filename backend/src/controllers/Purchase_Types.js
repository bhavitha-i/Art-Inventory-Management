...
exports.create = (req, res) => {
  ...
  dbmodels.Purchase_Types.create(rowData)
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.findAll = (req, res) => {
  ...
  dbmodels.Purchase_Types.findAll({})
      .then(result => {
        res.send(result);
      })
      .catch(err => {
...
exports.findByPk = (req, res) => {
    dbmodels.Purchase_Types.findByPk(req.params.id, {
    })
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.update = (req, res) => {
  ...
  dbmodels.Purchase_Types.update(req.body,
    { where: { id_Purchase_Types: id } }
  ).
  then(() => {
    res.status(200).json({
        status: true,
...
exports.delete = (req, res) => {
  const id = req.params.id;
  dbmodels.Purchase_Types.destroy({
    where: { id_Purchase_Types: id },
  }).then(() => {
    res.status(200).json({
        status: true,
