...
exports.create = (req, res) => {
  ...
  dbmodels.Art_Show.create(rowData)
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.findByPk = (req, res) => {
    dbmodels.Art_Show.findByPk(req.params.id, {
    })
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.update = (req, res) => {
  ...
  dbmodels.Art_Show.update(req.body,
    { where: { id_Art_Show: id } }
  ).
  then(() => {
    res.status(200).json({
        status: true,
