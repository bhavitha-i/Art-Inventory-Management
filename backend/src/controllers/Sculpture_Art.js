...
exports.create = (req, res) => {
  ...
  dbmodels.Sculpture_Art.create(rowData)
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.findAll = (req, res) => {
  ...
  dbmodels.Sculpture_Art.findAll({})
      .then(result => {
        res.send(result);
      })
      .catch(err => {
...
exports.findByPk = (req, res) => {
    dbmodels.Sculpture_Art.findByPk(req.params.id, {
    })
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.update = (req, res) => {
  ...
  dbmodels.Sculpture_Art.update(req.body,
    { where: { id_Sculpture_Art: id } }
  ).
  then(() => {
    res.status(200).json({
        status: true,
...
exports.delete = (req, res) => {
  const id = req.params.id;
  dbmodels.Sculpture_Art.destroy({
    where: { id_Sculpture_Art: id },
  }).then(() => {
    res.status(200).json({
        status: true,
