...
exports.create = (req, res) => {
  ...
  dbmodels.Payment_Status.create(rowData)
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.findAll = (req, res) => {
  ...
  dbmodels.Payment_Status.findAll({})
      .then(result => {
        res.send(result);
      })
      .catch(err => {
...
exports.findByPk = (req, res) => {
    dbmodels.Payment_Status.findByPk(req.params.id, {
    })
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.update = (req, res) => {
  ...
  dbmodels.Payment_Status.update(req.body,
    { where: { id_Payment_Status: id } }
  ).
  then(() => {
    res.status(200).json({
        status: true,
...
exports.delete = (req, res) => {
  const id = req.params.id;
  dbmodels.Payment_Status.destroy({
    where: { id_Payment_Status: id },
  }).then(() => {
    res.status(200).json({
        status: true,
