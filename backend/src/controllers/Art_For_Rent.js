...
exports.create = (req, res) => {
  ...
  dbmodels.Art_For_Rent.create(rowData)
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.findByPk = (req, res) => {
    dbmodels.Art_For_Rent.findByPk(req.params.id, {
    })
      .then((result) => {
      res.status(200).json({
        status: true,
...
exports.update = (req, res) => {
  ...
  dbmodels.Art_For_Rent.update(req.body,
    { where: { idArt_For_Rent: id } }
  ).
  then(() => {
    res.status(200).json({
        status: true,
